...
class WhatsAppMessageRequest extends Controller
{
    ...
    public function __construct(
        protected WhatsAppMessagePayloadService $messagePayloadService,
    ) {
    ...
    {
        $isTemplate = $requestType !== 'C';
        $effectiveMedia = filled($mediaID) ? (string) $mediaID : (filled($mediaLink) ? (string) $mediaLink : null);
        $resolvedMediaType = filled($mediaID) ? 'id' : (($mediaLink !== null && $mediaLink !== '') && is_numeric($mediaLink) ? 'id' : 'link');
        $template = is_array($templateData) ? $templateData : [];
        ...
        return $this->messagePayloadService->generate(
            ...
            $template,
            $customMessage !== null ? (string) $customMessage : null,
            is_array($dynamicValues) ? array_values($dynamicValues) : [],
            is_array($ButtonValue) ? array_values($ButtonValue) : [],
